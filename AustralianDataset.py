...
def UtastarAustralianfunc(df):
    ##################### UTASTAR CREDIT SCORE #########################

    (epsilon,performanceTable,alternativesRanks,criteriaMinMax,bpdata,criteriaNumberOfBreakPoints)=mcdainput(df)
...
def main():
    df = australiaDataLoadPreprossec()

    UtastarAustralianfunc(df)