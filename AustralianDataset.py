...
def accurMatrix(true_v,pred,prnt=False,co=True):
...
def DataUtastar(df,prnt=False):
    ...
    # valurfrand.to_excel(
    #    datapath + r"\valuefunc.xlsx"
    # )

    valurfrand = pd.read_excel(
        datapath+ "\Valuefunc.xlsx",
        sheet_name="Sheet1",
        #sheet_name="multimatrix",
    )
...
def DataTopsis(df,weights,prnt=False):
    ...
    if (prnt==True):
        print("Topsis Max Overall Value -->",max(topsisdf["Solution"]))
        print("Topsis Min Overall Value -->",min(topsisdf["Solution"]))
...
#Utastar
(valuefunc,utastardf) = DataUtastar(df.copy())

#Utadis
(valuefunc2,utadisdf) = DataUtadis(df.copy(),valuefunc,)

#Topsis -Utastar
print("--Topsis with Utastar Weights--")
utastar_topsis = DataTopsis(df.copy(),valuefunc)

#Topsis - Utadis
print("--Topsis with Utadis weights--")
utadis_topsis = DataTopsis(df.copy(),valuefunc2)
